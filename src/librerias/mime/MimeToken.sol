...
contract MimeToken is Initializable, OwnableUpgradeable, IMimeToken {
    ...
    /**
     * @custom:problema
     * QUe pasa si la ronda pasa y no se sete la merkleroot?
     * @custom:solucion
     * 
     * _round  ===  round()
     * _round  === round() +1
     ...
     * _round > round +1
     */
    function setNewRound(bytes32 merkleRoot_,uint _round) public onlyOwner {
        uint _curr= round();
        uint256 nextRound = round() + 1;
        if (_round<_curr) revert InvalidRound();
        else if (_round>nextRound)revert InvalidRound();
        else {
            /**
             * _round == round() || _round == round()+1
             */
            require(
                _merkleRootAt[_round] == bytes32(0), "MimeTokenWithDuration: merkle root already set for the next round"
            );
            _merkleRootAt[_round] = merkleRoot_;

            emit NewRound(_round, merkleRoot_);
        }
    }
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) public {
        ...
        /**
         * @custom:problema
         * El encoding que se usa es incorrecto, hay que cambiarlo y armar un nuevo set de pruebas
        */
